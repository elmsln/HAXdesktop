 ...
 */
import{LitElement,html,css}from"../../lit-element/lit-element.js";import"./lib/hex-a-gon.js";class HexagonLoader extends LitElement{static get styles(){return[css`
        :host {
          display: none;
        }
    ...
    </div>`}static get properties(){return{...super.properties,color:{name:"color",type:String,reflect:!0},size:{name:"size",type:String,reflect:!0},loading:{name:"loading",type:Boolean,reflect:!0},items:{name:"items",type:Array},itemCount:{name:"itemCount",type:Number,reflect:!0,attribute:"item-count"}}}static get tag(){return"hexagon-loader"}constructor(){super(),this.itemCount=37,this.items=[]}updated(changedProperties){changedProperties.forEach((oldValue,propName)=>{if("color"==propName&&this._colorChanged(this[propName],oldValue),"itemCount"==propName){this.items=[];for(let i=0;i<this[propName];i++)this.items.push("");let ctr=-4,increment=255/7,height=0,order=0,rownum=0,offset=0;[4,5,6,7,6,5,4].forEach((row,i)=>{ctr+=row,this.itemCount>=ctr&&(height=increment*(i+1),order=this.itemCount-ctr,rownum=i+1)}),offset=rownum>3?4:rownum>2?3:rownum>1?2:1,this.style.setProperty("--hexagon-loader-height",`${height}px`)}})}_colorChanged(newValue,oldValue){newValue&&window.ShadyCSS&&window.ShadyCSS.styleSubtree(this,{"--hexagon-color":newValue})}}window.customElements.define(HexagonLoader.tag,HexagonLoader);export{HexagonLoader};
...
_this.haxRayMode=!1;_this.activeNode=null;_this.haxSelectedText="";_this.activeContainerNode=null;setTimeout(function(){new Promise(function(res,rej){return _require.default(["./lib/hax-text-context.js"],res,rej)});new Promise(function(res,rej){return _require.default(["./lib/hax-ce-context.js"],res,rej)});new Promise(function(res,rej){return _require.default(["./lib/hax-plate-context.js"],res,rej)});new Promise(function(res,rej){return _require.default(["../grid-plate/grid-plate.js"],res,rej)});_this.polyfillSafe=window.HaxStore.instance.computePolyfillSafe();_this.addEventListener("hax-context-item-selected",_this._haxContextOperation.bind(babelHelpers.assertThisInitialized(_this)));_this.addEventListener("place-holder-replace",_this.replacePlaceholder.bind(babelHelpers.assertThisInitialized(_this)));_this.addEventListener("focusin",_this._focusIn.bind(babelHelpers.assertThisInitialized(_this)));_this.addEventListener("mousedown",_this._focusIn.bind(babelHelpers.assertThisInitialized(_this)));_this.addEventListener("dragenter",_this.dragEnterBody.bind(babelHelpers.assertThisInitialized(_this)));_this.addEventListener("drop",_this.dropEvent.bind(babelHelpers.assertThisInitialized(_this)));_this.addEventListener("click",_this.clickEvent.bind(babelHelpers.assertThisInitialized(_this)))},0);return _this}babelHelpers.createClass(HaxBody,[{key:"clickEvent",value:function clickEvent(e){// failsafe to clear to the gravity scrolling
clearTimeout(gravityScrollTimer)}/**
   * Activation allowed from outside this grid as far as drop areas
   */},{key:"dragEnterBody",value:function dragEnterBody(e){var children=this.childNodes;// walk the children and apply the draggable state needed
for(var i in children){if(children[i].classList&&children[i]!==this.activeItem){children[i].classList.add("hax-mover")}}}/**
   * LitElement render
   */},{key:"render",value:function render(){return(0,_litElement.html)(_templateObject2_a24402d0afe111eab5c3b1e04e313b03())}/**
   * LitElement / popular convention
   */},{key:"firstUpdated",/**
   * LitElement life cycle - ready
   */value:function firstUpdated(changedProperties){var _this2=this;if(babelHelpers.get(babelHelpers.getPrototypeOf(HaxBody.prototype),"firstUpdated",this)){babelHelpers.get(babelHelpers.getPrototypeOf(HaxBody.prototype),"firstUpdated",this).call(this,changedProperties)}this.dispatchEvent(new CustomEvent("hax-register-body",{bubbles:!0,cancelable:!0,composed:!0,detail:this}));// try to normalize paragraph insert on enter
...
else if(!window.HaxStore.instance.isGridPlateElement(containerNode)){activeNode=containerNode}// ensure this is a tag we care about / have support for and
...
if(this._HTMLPrimativeTest(node)){if(status){node.setAttribute("contenteditable",status)}else{node.removeAttribute("contenteditable")}if(0<node.querySelectorAll("a").length){for(var links=node.querySelectorAll("a"),j=0,len2=links.length;j<len2;j++){if(status){links[j].setAttribute("contenteditable",status);links[j].setAttribute("data-editable",status)}else{links[j].removeAttribute("contenteditable");links[j].removeAttribute("data-editable")}links[j][listenerMethod]("click",function(e){e.preventDefault();e.stopPropagation();e.stopImmediatePropagation()})}}}}/**
   ...
   */},{key:"dropEvent",value:function dropEvent(e){var _this18=this;if(!this.openDrawer&&this.editMode){// esnure we clear the gravity scrolling drag effect
clearTimeout(gravityScrollTimer);window.HaxStore.instance._lockContextPosition=!1;// trick the tray into forcing active to be Configure
window.HaxStore.instance.haxTray.activeTab="item-1";var target=null;if(e.path&&e.path[0]){target=e.path[0]}else{target=e.target}// establish an activeNode /container based on drop poisition
this.activeNode=target;window.HaxStore.write("activeNode",target,this);if(target.parentNode&&"GRID-PLATE"===target.parentNode.tagName){this.activeContainerNode=target.parentNode;window.HaxStore.write("activeContainerNode",target.parentNode,this)}else{this.activeContainerNode=target.parentNode;window.HaxStore.write("activeContainerNode",target,this)}// walk the children and remove the draggable state needed
...
target=window.HaxStore.instance.__dragTarget;var local=e.target;// if we have a slot on what we dropped into then we need to mirror that item
...
nextScrollX=Math.max(0,Math.min(maxScrollX,nextScrollX));nextScrollY=Math.max(0,Math.min(maxScrollY,nextScrollY));if(nextScrollX!==currentScrollX||nextScrollY!==currentScrollY){window.scrollTo(nextScrollX,nextScrollY);return!0}else{return!1}}}/**
   ...
   */},{key:"dragLeave",value:function dragLeave(e){if(!this.openDrawer&&this.editMode&&e.target&&e.target.classList){e.target.classList.remove("hax-hovered")}}/**
...
while(null!=node&&!this._validElementTest(node)){node=node.previousSibling}}if(null!=node){setTimeout(function(){node.focus()},50)}}}catch(e){console.warn(e)}}}}],[{key:"tag",get:function get(){return"hax-body"}},{key:"properties",get:function get(){return _objectSpread(_objectSpread({},babelHelpers.get(babelHelpers.getPrototypeOf(HaxBody),"properties",this)),{},{openDrawer:{type:Object},/**

...
*/var AppDatepicker=(0,_polymerLegacy.Polymer)({_template:(0,_polymerLegacy.html)(_templateObject_a0c10c50afe111eab5c3b1e04e313b03()),is:"app-datepicker",behaviors:[_neonAnimationRunnerBehavior.NeonAnimationRunnerBehavior],properties:{/**
     * Set locale for the datepicker.
     */locale:{type:String,value:function value(){if(window.Intl){return window.Intl&&window.Intl.DateTimeFormat&&window.Intl.DateTimeFormat().resolvedOptions&&window.Intl.DateTimeFormat().resolvedOptions().locale||"en-US"}return"en-US"}},/**
     ...
     */disableDays:{type:Array,value:function value(){return[6,0]}},/**
     * Array of dates need to be disabled for selection.
     */disableDates:{type:Array,value:function value(){return[]}},/**
...
_computeDaysOfMonth:function _computeDaysOfMonth(_activeYear,_activeMonth,_firstDayOfWeek,_locale){// No op for Intl is undefined (for Safari below v10.x.x).
if(!window.Intl){return}// Compute total number of days for a month.
function _computeTotalDaysOfMonth(_year,_month){var _totalDaysOfMonth=new Date(_year,_month+1,0).getDate();return _totalDaysOfMonth}var _start=new Date(_activeYear,_activeMonth,1).getDay(),_daysOfMonth=[],_totalDays=_computeTotalDaysOfMonth(_activeYear,_activeMonth);//  if _firstDayOfWeek is greater than 0 but less than 7.
if(0<_firstDayOfWeek&&7>_firstDayOfWeek){_start=_start-_firstDayOfWeek;_start=0>_start?7+_start:_start}// Set up Intl.
_locale=_locale||window.Intl&&window.Intl.DateTimeFormat&&window.Intl.DateTimeFormat().resolvedOptions&&window.Intl.DateTimeFormat().resolvedOptions().locale||"en-US";for(var _formatter=window.Intl&&window.Intl.DateTimeFormat?new window.Intl.DateTimeFormat(_locale,{timeZone:"UTC",day:"numeric"}).format:function dateTimeFormatShim(date){return date.getDate()},i=0,j=1-_start;42>i;i++,j++){var _formatted=_formatter(Date.UTC(_activeYear,_activeMonth,j)),_dateObj={date:"",index:""};if(i>=_start&i<_start+_totalDays){/**
         * NOTE: Coercing '_formatted' to a number will cause
         * dates in other languages that contain other character
         * to become a NaN.
         */_dateObj.date=_formatted;_dateObj.index=j}_daysOfMonth.push(_dateObj)}// Update _chosenDaysOfMonth for every new calendar being rendered.
this.set("_chosenDaysOfMonth",this._computeChosenDaysOfMonth(_daysOfMonth,this._selectedDate));// Update the old _daysOfMonth.
this.set("_daysOfMonth",_daysOfMonth)},/// Re-compute disable days with firstDayOfWeek.
_computeShiftedDisableDays:function _computeShiftedDisableDays(_firstDayOfWeek,_isDisableDays){_firstDayOfWeek=0<_firstDayOfWeek&&7>_firstDayOfWeek?_firstDayOfWeek:0;var _sdd=this.disableDays.map(function(_day){_day=_day-_firstDayOfWeek;return 0>_day?7+_day:_day});this.set("_shiftedDisableDays",_sdd)},// Increment the month via user interaction.
_incrementMonth:function _incrementMonth(ev){this.debounce("_incrementMonth",function(){this.set("_isIncrementMonth",!0);window.requestAnimationFrame(function(){var _month=this._activeMonth;if(0===++_month%12){this._activeYear++}this.set("_activeMonth",_month%12);if(!this.noAnimation){this.cancelAnimation();this.playAnimation("incrementEntry")}this.set("_isIncrementMonth",!1)}.bind(this))},100)},// Decrement the month via user interaction.
_decrementMonth:function _decrementMonth(ev){this.debounce("_decrementMonth",function(){this.set("_isDecrementMonth",!0);window.requestAnimationFrame(function(){var _month=this._activeMonth;if(0>--_month){this._activeYear--;_month=11}this.set("_activeMonth",_month);if(!this.noAnimation){this.cancelAnimation();this.playAnimation("decrementEntry")}this.set("_isDecrementMonth",!1)}.bind(this))},100)},// Re-compute active month year if new date is selected.
_computeActiveMonthYear:function _computeActiveMonthYear(_activeYear,_activeMonth,_locale){if(window.Intl){_locale=_locale||window.Intl&&window.Intl.DateTimeFormat&&window.Intl.DateTimeFormat().resolvedOptions&&window.Intl.DateTimeFormat().resolvedOptions().locale||"en-US";var _amy=new window.Intl.DateTimeFormat(_locale,{timeZone:"UTC",month:"short",year:"numeric"}).format(Date.UTC(_activeYear,_activeMonth,1));this.set("_activeMonthYear",_amy)}},// Re-compute short version of full date to show on the picker.
_computeShortSelectedDate:function _computeShortSelectedDate(_selectedYear,_selectedMonth,_selectedDate,_locale){if(window.Intl){_locale=_locale||window.Intl&&window.Intl.DateTimeFormat&&window.Intl.DateTimeFormat().resolvedOptions&&window.Intl.DateTimeFormat().resolvedOptions().locale||"en-US";var _ssd=new window.Intl.DateTimeFormat(_locale,{timeZone:"UTC",weekday:"short",month:"short",day:"numeric"}).format(Date.UTC(_selectedYear,_selectedMonth,_selectedDate));this.set("_shortSelectedDate",_ssd);// When datepicker has no button for date confirmation,
// by asserting `autoUpdateDate` will trigger auto update on
// date change.
if(this.autoUpdateDate){this.enforceDateChange()}}},_computeShowSelectedYear:function _computeShowSelectedYear(_selectedYear,_locale){if(window.Intl){_locale=_locale||window.Intl&&window.Intl.DateTimeFormat&&window.Intl.DateTimeFormat().resolvedOptions&&window.Intl.DateTimeFormat().resolvedOptions().locale||"en-US";var _ssy=new window.Intl.DateTimeFormat(_locale,{timeZone:"UTC",year:"numeric"}).format(Date.UTC(_selectedYear,0,1));this.set("_showSelectedYear",_ssy)}},// When a day of month is selected, a lot of tasks need to be carried out.
_chooseDaysOfMonth:function _chooseDaysOfMonth(ev){var _target=ev.target;// daysOfMonth is chooseable when:
// a) _target.date is of type Number,
// b) _target.classList.contains('is-disabled-day').
if(_target&&this._isNumber(_target.date)&&!_target.classList.contains("is-disabled-day")){// This will trigger _isChosenDaysOfMonth to recompute style.
this.set("_chosenDaysOfMonth",_target.index);this.set("_selectedYear",this._activeYear);this.set("_selectedDate",_target.date);this.set("_selectedMonth",this._activeMonth)}},// Re-compute class when the item (day of month) is today's date.
_isToday:function _isToday(_item,_activeYear,_activeMonth){var _now=new Date,_isToday=_item===_now.getDate()&&_activeYear===_now.getFullYear()&&_activeMonth===_now.getMonth();return _isToday?" is-today":""},// Re-compute class when the item (day of month) is empty date & non-selectable.
_isEmptyDate:function _isEmptyDate(_item){return this._isNumber(_item)?"":" is-non-selectable"},// Re-compute class if the item (day of month) is the selected date.
_isChosenDaysOfMonth:function _isChosenDaysOfMonth(_item,_selectedYear,_selectedMonth,_selectedDate){// able to retain selected daysOfMonth even after navigating to other months.
var _isChosenDaysOfMonth=0<=this._chosenDaysOfMonth&&this._activeYear===_selectedYear&&this._activeMonth===_selectedMonth&&_item===_selectedDate;return _isChosenDaysOfMonth?" chosen-days-of-month":""},// Re-compute class if the item (day of month) is a disable day.
_isDisableDays:function _isDisableDays(_index,_firstDayOfWeek,_minDate,_maxDate,_item){var _isLessThanMinDate=!1,_isMoreThanMaxDate=!1,_isDisableDay=!1,_isDisableDates=!1,_isDisableDays=this._shiftedDisableDays.some(function(_n){return _n===_index%7});// ------ < _minDate ---------------- _maxDate > ------
// if _item is of type Number.
// if converted _item into new Date() < minDate or > maxDate.
if(this._isNumber(_item)){var _minDateObj=this._convertDateStringToDateObject(_minDate),_maxDateObj=this._convertDateStringToDateObject(_maxDate),_currentDate=new Date(this._activeYear,this._activeMonth,_item);// run two different obj differently just in case only one of them
// is defined and still be able to update disabled days.
if(_minDateObj){_isLessThanMinDate=_currentDate.getTime()<_minDateObj.getTime()}if(_maxDateObj){_isMoreThanMaxDate=_currentDate.getTime()>_maxDateObj.getTime()}_isDisableDates=this.disableDates&&this.disableDates.length&&this.disableDates.some(function(date){var _dateObj=this._convertDateStringToDateObject(date);return _dateObj&&_currentDate.getTime()===_dateObj.getTime()}.bind(this))}// To disable a date, it must be either one of the followings:
...
_isDisableDay=_isDisableDays||_isDisableDates||_isLessThanMinDate||_isMoreThanMaxDate;return _isDisableDay?" is-disabled-day is-non-selectable":""},// Re-compute class when a year is selected at year page.
_isListOfYearsSelected:function _isListOfYearsSelected(selectedYear,year){return+selectedYear===+year?" is-selected":""},_computeDaysOfWeek:function _computeDaysOfWeek(_firstDayOfWeek,_locale){// _daysOfWeek needs to be changed as well with firstDayOfWeek and locale.
var _dow=["S","M","T","W","T","F","S"],_firstDayOfWeek=0<_firstDayOfWeek&&7>_firstDayOfWeek?_firstDayOfWeek:0;// if locale is set and window.Intl is supported...
if(_locale&&window.Intl){_dow=[];for(var _today=new Date,_offsetDate=_today.getDate()-_today.getDay(),_formatter=new window.Intl.DateTimeFormat(_locale,{timeZone:"UTC",weekday:"narrow"}).format,newDate=null,i=0;7>i;i++){newDate=Date.UTC(_today.getFullYear(),_today.getMonth(),_offsetDate+i);_dow.push(_formatter(newDate))}}var _sliced=_dow.slice(_firstDayOfWeek),_rest=_dow.slice(0,_firstDayOfWeek),_concatted=Array.prototype.concat(_sliced,_rest);this.set("_daysOfWeek",_concatted)},// Re-compute the index of the selected date when a new date is selected
// via user interaction.
_computeChosenDaysOfMonth:function _computeChosenDaysOfMonth(_daysOfMonth,_selectedDate){// depends on _daysOfMonth and recalculate the index of _chosenDaysOfMonth.
for(var _len=_daysOfMonth.length,i=0;i<_len;i++){if(0<=i&&_daysOfMonth[i].index===_selectedDate){return i}}},// Convert date string into date object.
_convertDateStringToDateObject:function _convertDateStringToDateObject(_date){var _checkDate=babelHelpers.instanceof(_date,Date)||"string"!==typeof _date?_date:new Date(_date),_isValidDate="Invalid Date"!==_checkDate.toDateString();return _isValidDate?_checkDate:null},// Dynamically load items into the iron-list when switching page to year view.
_updateList:function _updateList(_activeView){for(var _newList=[],y=1900,i=y;2100>=i;i++){_newList.push({year:i})}this.set("_listOfYears",_newList)},// On neon-animaition-finish, udpate scroller position of the iron-list.
// It is to fix the iron-list so that the selected year will always be centered by
// hardcoded with fixed length of #items.
_onAnimationFinish:function _onAnimationFinish(ev){var _this=this,_target=ev.detail;// TO fix the scrolling of iron-list, hard coded with fixed length of #items.
if(_target&&"IRON-LIST"===_target.toPage.tagName){var _focusableItem=this._updateListScroller(_target.toPage);// Automatically focus selected list of year at year view.
async.microTask.run(function(){_target.toPage._focusPhysicalItem(_focusableItem)})}else{// When user navigate to calendar view from list view,
// re-focus showSelectedYear.
async.microTask.run(function(){_this.$.showSelectedYear.focus()})}},_updateListScroller:function _updateListScroller(_list){var _this2=this,_sl=(0,_polymerDom.dom)(_list.root).querySelector("#items"),_slh=_sl.getBoundingClientRect().height||12863.994140625,_sli=Math.floor(_slh/(2100-1900+1)*(this._activeYear-1900-2))+1;// Adjust center position when datepickers has no buttons and in landscape mode.
if(!this.$.dp.classList.contains("with-buttons")&&window.matchMedia("(orientation: landscape)").matches){_sli=Math.floor(_slh/(2100-1900+1)*(this._activeYear-1900-1))+1}// Scroll to selected year.
async.microTask.run(function(){_list.scroll(0,_sli);// Select initial item for the list.
_list.selectItem(_this2._activeYear-1900)});// wait for at least 17ms to update the scroller position.
// Return index of item that should be focused on the list.
return this._activeYear-1900},// When first time switching page to year view, dynamically render iron-list.
_onIronSelectorSelectedChanged:function _onIronSelectorSelectedChanged(ev){if("year"===ev.detail.value){if(!this._isListRendered){this._updateList();this.set("_isListRendered",!0)}else{// When noAnimation is truthy, this will update the list scroller.
if(this.noAnimation){this._updateListScroller(this.shadowRoot.querySelector("#listOfYears"))}}}},_onListRendered:function _onListRendered(ev){var _this3=this;// When noAnimation is truthy, this will update the list scroller.
if(ev.target.if&&this.noAnimation){async.microTask.run(function(){_this3._updateListScroller(_this3.shadowRoot.querySelector("#listOfYears"))})}},_goCalendar:function _goCalendar(ev){// Listen for enter key from keyboard.
if("keydown"===ev.type&&13!==ev.keyCode){return}var _selectedYear=ev.model.item.year;// Set _activeYear and _selectedYear to the selected year.
this.set("_activeYear",_selectedYear);this.set("_selectedYear",_selectedYear);// Update selected item in iron-list.
this.shadowRoot.querySelector("#listOfYears").selectItem(_selectedYear-1900);// Go back to calendar page.
this.set("_activeView","calendar")},// split capturing group of format into year, month and date.
_computeSeparateFormat:function _computeSeparateFormat(_format){var re=/^(yyyy|yy|m{1,4}|d{1,2}|do)\W+(yyyy|yy|m{1,4}|d{1,2}|do)\W+(yyyy|yy|m{1,4}|d{1,2}|do)$/g,m=re.exec(_format),_temp={},_tempArr=[];if(null!==m){_tempArr.push(m[1]);_tempArr.push(m[2]);_tempArr.push(m[3]);for(var i=0,matched;3>i;i++){matched=_tempArr[i];if(0<=matched.indexOf("y")){_temp.y=matched}else if(0<=matched.indexOf("m")){_temp.m=matched}else if(0<=matched.indexOf("d")){_temp.d=matched}}}// Only set _format if the new format is valid.
if("d"in _temp&&"m"in _temp&&"y"in _temp){this.set("_format",_temp)}_temp=null},// bind the selected date so that it's ready to be read from the outside world.
_bindSelectedFulldate:function _bindSelectedFulldate(_selectedYear,_selectedMonth,_selectedDate,_format){// show long date (eg. Fri, May 12 2017 instead.
if(this.showLongDate){return this._computeShowLongDate(this.showLongDate,this.locale,!0)}var _formattedYear=_selectedYear,_formattedMonth=this._monthNames[_selectedMonth],_formattedDate=_selectedDate,_finalFormatted=this.format;// compute new formatted year.
if("yy"===_format.y){_formattedYear=_selectedYear%100}// compute new formatted month.
if("mmm"===_format.m){_formattedMonth=_formattedMonth.slice(0,3)}else if("mm"===_format.m){_formattedMonth=this._padStart(_selectedMonth+1,2,"0")}else if("m"===_format.m){_formattedMonth=_selectedMonth+1}// compute new formatted date.
if("do"===_format.d){var _cardinalNumber=_formattedDate%10,_suffixOrdinal=3<_cardinalNumber?"th":["th","st","nd","rd"][_cardinalNumber];if(11===_formattedDate||12==_formattedDate||13===_formattedDate){_suffixOrdinal="th"}_formattedDate=_formattedDate+_suffixOrdinal}else if("dd"===_format.d){_formattedDate=this._padStart(_formattedDate,2,"0")}// set formatted value with user defined symbols.
_finalFormatted=_finalFormatted.replace(_format.y,_formattedYear);_finalFormatted=_finalFormatted.replace(_format.m,_formattedMonth);_finalFormatted=_finalFormatted.replace(_format.d,_formattedDate);return _finalFormatted},// method for content tag (eg. buttons).
_updateBindDate:function _updateBindDate(ev){this.debounce("_updateBindDate",function(){var _type=ev.type;if("tap"===_type){this.set("_isSelectedDateConfirmed",!0)}if("transitionend"===_type||this.noAnimation){if(this._isSelectedDateConfirmed){var _sy=this._selectedYear,_sm=this._selectedMonth,_sd=this._selectedDate,_f=this._format,_confirmDate=this._bindSelectedFulldate(_sy,_sm,_sd,_f);this._setDate(_confirmDate);this.set("_isSelectedDateConfirmed",!1);if(this.alwaysResetSelectedDateOnDialogClose){this.resetDate()}}}},1)},// Update date to show long date or short date.
_computeShowLongDate:function _computeShowLongDate(_showLongDate,_locale,_returnResult){if(!window.Intl||!this._selectedDate||"undefined"===typeof this._selectedDate){return}var _selectedDate=this._selectedDate,_longDate=Date.UTC(this._selectedYear,this._selectedMonth,_selectedDate);if(_showLongDate){_locale=_locale||window.Intl&&window.Intl.DateTimeFormat&&window.Intl.DateTimeFormat().resolvedOptions&&window.Intl.DateTimeFormat().resolvedOptions().locale||"en-US";var _options={timeZone:"UTC",weekday:this.showLongDate?"short":void 0,year:"numeric",month:this.showLongDate?"short":"2-digit",day:"2-digit"};_longDate=new window.Intl.DateTimeFormat(_locale,_options).format(_longDate);if(_returnResult){return _longDate}// Fix for notorious IE - add unknown spaces (%E2%80%8E) when formatting date with Intl.
// http://utf8-chartable.de/unicode-utf8-table.pl?start=8192&number=128
if(window.navigator.msManipulationViewsEnabled){if(_locale||""===_locale){_longDate=decodeURIComponent(encodeURIComponent(_longDate).replace(/\%E2\%80\%8[0-9A-F]/gi,""))}}this._setDate(_longDate)}else{_longDate=this._bindSelectedFulldate(this._selectedYear,this._selectedMonth,_selectedDate,this._format);if(_returnResult){return _longDate}this._setDate(_longDate)}},// TO make things simple and works across different browsers, the input date string
// has to be standardized - YYYY/MM/DD.
// http://dygraphs.com/date-formats.html
_updateToReflectExternalChange:function _updateToReflectExternalChange(_inputDate){// Due to limitation of input date received by platform's Date,
...
var _re1=/^(\d{4})\W+(\d{1,2})\W+(\d{1,2})$/i,_re2=/^(\d{4})[ ](\w+)[ ](\d{1,2})$/i,_validWithRe1=_re1.exec(_id),_validWithRe2=_re2.exec(_id);if(null===_validWithRe1&&null===_validWithRe2){return _res}else{var _resultToDate=null;if(null===_validWithRe1){_resultToDate=new Date(_validWithRe2[1]+" "+_validWithRe2[2]+" "+_validWithRe2[3])}else if(null===_validWithRe2){_resultToDate=new Date(+_validWithRe1[1],+_validWithRe1[2]-1,+_validWithRe1[3])}return{valid:!0,result:_resultToDate}}}var _showLongDate=this.showLongDate,_yy=0,_mm=0,_dd=0,_isValidDate=validateDate(_inputDate,_showLongDate);if(_isValidDate.valid){if(this.alwaysResetSelectedDateOnDialogClose){return}var _vd=new Date(_isValidDate.result),_yy=_vd.getFullYear(),_mm=_vd.getMonth();this._setInvalidDate(!1);this.set("_activeYear",_yy);this.set("_activeMonth",_mm);this.set("_selectedYear",_yy);this.set("_selectedMonth",_mm);this.set("_selectedDate",_vd.getDate())}else{this.set("_selectedDate",this._selectedDate||new Date().getDate());this._computeShowLongDate(_showLongDate,this.locale);this._setInvalidDate(!0)}},/**
   ...
   */enforceDateChange:function enforceDateChange(){this._setInvalidDate(!1);this._computeShowLongDate(this.showLongDate,this.locale)},// reset date to today's date if alwaysResetSelectedDateOnDialogClose is set.
resetDate:function resetDate(){if(this._isSelectedDateConfirmed){return}var now=new Date,nowFy=now.getFullYear(),nowM=now.getMonth(),nowD=now.getDate();this.set("_activeYear",nowFy);this.set("_activeMonth",nowM);this.set("_selectedYear",nowFy);this.set("_selectedMonth",nowM);this.set("_selectedDate",nowD);this._setInvalidDate(!1)},// Accessibility enhancment.
_shouldTabIndex:function _shouldTabIndex(_index,_firstDayOfWeek,_minDate,_maxDate,_item){var _isDisableDays=this._isDisableDays(_index,_firstDayOfWeek,_minDate,_maxDate,_item);return _item&&0<=_item&&!_isDisableDays?0:-1},_shouldAriaDisabled:function _shouldAriaDisabled(_index,_firstDayOfWeek,_minDate,_maxDate,_item){var _isDisableDays=this._isDisableDays(_index,_firstDayOfWeek,_minDate,_maxDate,_item);return!(_item&&0<=_item&&!_isDisableDays)},// Lodash's replacements.
_padStart:function _padStart(_string,_length,_chars){var _len=-_length,_str=(_chars+_string).slice(_len);return _str},_isNumber:function _isNumber(_value){// return typeof _value == 'number' || (!Number.isNaN(parseFloat(_value)) && Number.isFinite(_value));
// Fallback: caused by IE11 as those methods are not supported in IE11.
return"number"==typeof _value||!isNaN(parseFloat(_value))&&isFinite(_value)},// Update theme color.
_updateThemeColor:function _updateThemeColor(_theme){var _themes=["dark-theme","light-theme","goog-theme"],_themeIdx=_themes.indexOf(_theme),_distributedButtons=(0,_polymerDom.dom)(this).querySelectorAll("paper-button"),_distributedButtonsLen=_distributedButtons.length,_colorCode=["#bcbcbc","#737373","#616161"][_themeIdx];if(0<=_themeIdx){_themes.splice(_themeIdx,1);this.toggleClass(_themes[0],!1,this);this.toggleClass(_themes[1],!1,this);this.toggleClass(_theme,!0,this)}else{this.toggleClass("dark-theme",!1,this);this.toggleClass("light-theme",!1,this);this.toggleClass("goog-theme",!1,this)}// workaround to update custom property of distributed buttons.
for(var i=0;i<_distributedButtonsLen;i++){this._updateDistributedButtonInkColorCustomProp(_distributedButtons[i],_colorCode||"#737373")}// Update styles of the whole datepicker.
this.updateStyles()},// Forcefully update the view of the datepicker.
_updateDatepickerView:function _updateDatepickerView(_view){this.toggleClass("horizontal-view","horizontal"===_view,this);this.toggleClass("vertical-view","vertical"===_view,this)},// workaround to update custom property of distributed children until Polymer supports Native custom properties.
_updateDistributedButtonInkColorCustomProp:function _updateDistributedButtonInkColorCustomProp(_node,_colorCode){this.updateStyles({"--paper-button-ink-color":_colorCode})}// TODO: To add GIF to README.md.

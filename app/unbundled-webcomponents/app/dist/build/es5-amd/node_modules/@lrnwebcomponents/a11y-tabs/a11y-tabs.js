 ...
 * @element a11y-tabs
 */var A11yTabs=/*#__PURE__*/function(_ResponsiveUtilityBeh){babelHelpers.inherits(A11yTabs,_ResponsiveUtilityBeh);babelHelpers.createClass(A11yTabs,[{key:"render",// render function
value:function render(){var _this2=this;return(0,_litElement.html)(_templateObject_d50a45e0812711eb9b3b3bc8a337bd59(),this.tabs.map(function(tab,i){return(0,_litElement.html)(_templateObject2_d50a45e0812711eb9b3b3bc8a337bd59(),_this2._tabButton(tab,i))}))}// haxProperty definition
}],[{key:"styles",//styles function
get:function get(){return[(0,_litElement.css)(_templateObject3_d50a45e0812711eb9b3b3bc8a337bd59())]}},{key:"haxProperties",get:function get(){return{canScale:!1,canPosition:!0,canEditSource:!0,gizmo:{title:"Tabs",description:"A set of tabs.",icon:"view-day",color:"grey",groups:["Content","Presentation","Tabs"]},settings:{configure:[{property:"ariaLabel",title:"Label (for accesibility)",inputMethod:"textfield"},{property:"disabled",title:"Disabled",inputMethod:"boolean"},{property:"hidden",title:"Hidden",inputMethod:"boolean"},{property:"sticky",title:"Sticky.",desc:"Horizontal tabs stick to the top of the window when scrolling.",inputMethod:"boolean"},{property:"layoutBreakpoint",title:"Layout Breakpoint",inputMethod:"Number",descripton:"Optional minimum breakpoint for horizontal layout of tabs. Default is unset (always horizontal). Setting `-1` forces vertical-only mode."},{property:"iconBreakpoint",title:"Icon Breakpoint",inputMethod:"Number",descripton:"Optional minimum breakpoint for showing tab text with icons. Default is always text with icons (0). Setting to -1 forces icon-only mode."},{slot:"",title:"Tabs",description:"A series of <a11y-tab/> elements.",inputMethod:"code-editor"}],advanced:[{property:"id",title:"Unique ID",inputMethod:"textfield"}]}}}// properties available to the custom element for data binding
},{key:"properties",get:function get(){return _objectSpread({},babelHelpers.get(babelHelpers.getPrototypeOf(A11yTabs),"properties",this),{ariaLabel:{type:String,reflect:!0,attribute:"aria-label"},/**
       * id of active tab
       */activeTab:{type:String,reflect:!0,attribute:"active-tab"},/**
       * if tabs should be full width or not
       */fullWidth:{type:Boolean,reflect:!0,attribute:"full-width"},/**
       * whether tabbed interface is disabled
       */disabled:{type:Boolean,reflect:!0,attribute:"disabled"},/**
       * whether tabbed interface is hidden
       */hidden:{type:Boolean,reflect:!0,attribute:"hidden"},/**
       ...
       */sticky:{type:Boolean,reflect:!0,attribute:"sticky"},/**
       * an array of tab data based on slotted `a11y-tab` elements
       */__tabs:{type:Array},/**
       * an array of tab buttons
       */__tabButtons:{type:Array},/**
       * which tab button has ketboard focus
       */__tabFocus:{type:Number}})}/**
   * Store the tag name to make it easier to obtain directly.
   * @notice function name must be here for tooling to operate correctly
   */},{key:"tag",get:function get(){return"a11y-tabs"}}]);function A11yTabs(){var _this;babelHelpers.classCallCheck(this,A11yTabs);_this=babelHelpers.possibleConstructorReturn(this,babelHelpers.getPrototypeOf(A11yTabs).call(this));_this.fullWidth=!1;_this.disableResponsive=!1;_this.disabled=!1;_this.hidden=!1;_this.__tabs=[];_this.__tabFocus=0;_this.addEventListener("a11y-tab-changed",function(e){return _this.updateTabs()});return _this}babelHelpers.createClass(A11yTabs,[{key:"connectedCallback",/**
   * life cycle, element is afixed to the DOM
   */value:function connectedCallback(){babelHelpers.get(babelHelpers.getPrototypeOf(A11yTabs.prototype),"connectedCallback",this).call(this);this.updateTabs();this.observer.observe(this,{attributes:!1,childList:!0,subtree:!1})}/**
   * life cycle, element is removed from the DOM
   */},{key:"disconnectedCallback",value:function disconnectedCallback(){if(this.observer&&this.observer.disconnect)this.observer.disconnect();babelHelpers.get(babelHelpers.getPrototypeOf(A11yTabs.prototype),"disconnectedCallback",this).call(this)}/**
   * handle updates
   */},{key:"updated",value:function updated(changedProperties){var _this3=this;changedProperties.forEach(function(oldValue,propName){if("id"===propName)_this3._idChanged(_this3.id,oldValue);if("activeTab"===propName&&_this3.activeTab!==oldValue)_this3._activeTabChanged(_this3.activeTab,oldValue);if("responsiveWidth"===propName){if(_this3.vertical){_this3.setAttribute("vertical",!0)}else{_this3.removeAttribute("vertical")}}if(["iconsBreakpoint","responsiveWidth","__tabs"].includes(propName)){if(_this3.iconsOnly){_this3.setAttribute("icons-only",!0)}else{_this3.removeAttribute("icons-only")}}})}/**
   * selects a tab
   * @param {string} id the active tab's id
   */},{key:"selectTab",value:function selectTab(id){var tabs=this.querySelectorAll(this.tabQuery);if(tabs&&0<tabs.length){var enabled=Object.keys(tabs||[]).filter(function(key){return!tabs[key].disabled}).map(function(key){return tabs[key].id}),filtered=enabled.filter(function(tabid){return tabid===id}),selected=filtered[0]||enabled[0];tabs.forEach(function(tab){tab.hidden=tab.id!==selected});this.activeTab=selected}}/**
   * updates the list of items based on slotted a11y-tab elements
   */},{key:"updateTabs",value:function updateTabs(e){this.__tabs=this.querySelectorAll(this.tabQuery);this.__tabButtons=this.shadowRoot.querySelectorAll("[role=tab]");this.selectTab(this.activeTab)}/**
   * Observer activeTab for changes
   * @param {string} newValue the new active tab's id
   */},{key:"_activeTabChanged",value:function _activeTabChanged(newValue,oldValue){if(newValue!==oldValue)this.selectTab(newValue);window.dispatchEvent(new CustomEvent("active-tab-changed",{bubbles:!0,cancelable:!0,composed:!0,detail:this}))}/**
   ...
   */},{key:"_handleTab",value:function _handleTab(tab){if(!tab.disabled)this.activeTab=tab.id}},{key:"_handleKey",value:function _handleKey(i,e){var _this4=this;this.__tabFocus=i;var focus=function focus(){var dir=0<arguments.length&&arguments[0]!==void 0?arguments[0]:1;_this4.__tabFocus=_this4.__tabFocus+dir;// If we're at the end, go to the start
if(_this4.__tabFocus>=_this4.buttons.length){_this4.__tabFocus=0;// If we're at the start, move to the end
}else if(0>_this4.__tabFocus){_this4.__tabFocus=_this4.buttons.length-1}if(_this4.buttons[_this4.__tabFocus].disabled&&_this4.__tabFocus!==i)focus(dir)};// Move right
if(39===e.keyCode||37===e.keyCode){this.buttons[i].setAttribute("tabindex",-1);focus(39===e.keyCode?1:-1);if(!this.buttons[this.__tabFocus].disabled){this.buttons[this.__tabFocus].setAttribute("tabindex",0);this.buttons[this.__tabFocus].focus()}}}/**
   ...
   * @returns object
   * @memberof A11yTabs
   */},{key:"_tabButton",value:function _tabButton(tab,i){var _this5=this;return(0,_litElement.html)(_templateObject4_d50a45e0812711eb9b3b3bc8a337bd59(),tab.id,tab.id===this.activeTab?"true":"false",tab.id,tab.id===this.activeTab&&!this.disabled?"active":"",tab.flag,function(e){return _this5._handleTab(tab)},function(e){return _this5._handleKey(i,e)},tab.disabled||this.disabled,tab.id===this.activeTab?0:-1,this._tabIcon(tab,"flagIcon"),this._tabLabel(tab),this._tabFlag(tab),this._tabIcon(tab,"icon"),this._tabTooltip(tab))}/**
   * makes tab flag
   *
   * @param {string} flag tab's flag
   * @returns object
   * @memberof A11yTabs
   */},{key:"_tabFlag",value:function _tabFlag(tab){return(0,_litElement.html)(_templateObject5_d50a45e0812711eb9b3b3bc8a337bd59(),!tab.flag,tab.flag)}/**
   * makes tab icon
   *
   * @param {string} icon tab's icon
   * @returns object
   * @memberof A11yTabs
   */},{key:"_tabIcon",value:function _tabIcon(tab,icon){return tab.flag?(0,_litElement.html)(_templateObject6_d50a45e0812711eb9b3b3bc8a337bd59(),!tab[icon],tab[icon],tab.flag):(0,_litElement.html)(_templateObject7_d50a45e0812711eb9b3b3bc8a337bd59(),!tab[icon],tab[icon])}/**
   * makes tab label
   *
   * @param {string} flag tab's flag
   * @returns object
   * @memberof A11yTabs
   */},{key:"_tabLabel",value:function _tabLabel(tab){return(0,_litElement.html)(_templateObject8_d50a45e0812711eb9b3b3bc8a337bd59(),tab.label)}/**
   * makes tab tooltip
   *
   ...
   * @returns object
   * @memberof A11yTabs
   */},{key:"_tabTooltip",value:function _tabTooltip(tab){return(0,_litElement.html)(_templateObject9_d50a45e0812711eb9b3b3bc8a337bd59(),tab.id,tab.label)}},{key:"buttons",get:function get(){return this.__tabButtons}/**
   * determines if tabs should show icons only
   * @readonly
   * @returns {boolean}
   */},{key:"iconsOnly",get:function get(){return this.iconBreakpoint&&1>(this.tabs||[]).filter(function(tab){return!tab.icon}).length&&this.responsiveWidth<this.iconBreakpoint}/**
   * mutation observer for tabs
   * @readonly
   * @returns {object}
   */},{key:"observer",get:function get(){var _this6=this,callback=function callback(){return _this6.updateTabs()};return new MutationObserver(callback)}/**
   * query selector for tabs
   * override this for custom elements that extend a11y-tabs
   *
   * @readonly
   * @memberof A11yTabs
   */},{key:"tabQuery",get:function get(){return"a11y-tab"}/**
   * array of tabs
   * @readonly
   * @returns {object}
   */},{key:"tabs",get:function get(){var _this7=this;return Object.keys(this.__tabs||{}).map(function(i){_this7.__tabs[i].order=i+1;_this7.__tabs[i].total=_this7.__tabs.length;return _this7.__tabs[i]})}/**
   * determines whether tabs should be in vertical layout
   * @readonly
   * @returns {boolean}
   */},{key:"vertical",get:function get(){return this.layoutBreakpoint&&this.layoutBreakpoint<this.responsiveWidth}}]);return A11yTabs}((0,_responsiveUtilityBehaviors.ResponsiveUtilityBehaviors)(_litElement.LitElement));_exports.A11yTabs=A11yTabs;window.customElements.define(A11yTabs.tag,A11yTabs)});
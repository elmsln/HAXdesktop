...
aaLarge:[{min:7,max:12},{min:7,max:12},{min:8,max:12},{min:9,max:12},{min:10,max:12},{min:11,max:12},{min:1,max:2},{min:1,max:3},{min:1,max:4},{min:1,max:5},{min:1,max:6},{min:1,max:6}],aa:[{min:8,max:12},{min:8,max:12},{min:9,max:12},{min:9,max:12},{min:11,max:12},{min:12,max:12},{min:1,max:1},{min:1,max:2},{min:1,max:4},{min:1,max:4},{min:1,max:5},{min:1,max:5}]}};return _this}/**
   * Store the tag name to make it easier to obtain directly.
   */babelHelpers.createClass(SimpleColorsSharedStyles,[{key:"getColorInfo",/**
   ...
   */value:function getColorInfo(colorName){var temp1=colorName.replace(/(simple-colors-)?(-text)?(-border)?/g,"").split("-theme-"),theme=0<temp1.length?temp1[0]:"default",temp2=0<temp1.length?temp1[1].split("-"):temp1[0].split("-"),color=1<temp2.length?temp2.slice(1,temp2.length-1).join("-"):"grey",shade=1<temp2.length?temp2[temp2.length-1]:"1";return{theme:theme,color:color,shade:shade}}/**
   ...
   */},{key:"makeVariable",value:function makeVariable(){var color=0<arguments.length&&arguments[0]!==void 0?arguments[0]:"grey",shade=1<arguments.length&&arguments[1]!==void 0?arguments[1]:1,theme=2<arguments.length&&arguments[2]!==void 0?arguments[2]:"default";return["--simple-colors",theme,"theme",color,shade].join("-")}/**
   ...
   */},{key:"getContrastingShades",value:function getContrastingShades(isLarge,colorName,colorShade,contrastName){var hasGrey="grey"===colorName||"grey"===contrastName?"greyColor":"colorColor",aa=isLarge?"aaLarge":"aa",index=parseInt(colorShade)+1,range=this.contrasts[hasGrey][aa][index];return Array(range.max-range.min+1).fill().map(function(_,idx){return range.min+idx})}/**
   * for large or small text given a color and its shade,
   ...
   */},{key:"getContrastingColors",value:function getContrastingColors(colorName,colorShade,isLarge){var _this2=this,result={};Object.keys(this.colors).forEach(function(color){result[color]=_this2.getContrastingShades(isLarge,colorName,colorShade,color)});return result.color}/**
   ...
   */},{key:"isContrastCompliant",value:function isContrastCompliant(isLarge,colorName,colorShade,contrastName,contrastShade){var hasGrey="grey"===colorName||"grey"===contrastName?"greyColor":"colorColor",aa=isLarge?"aaLarge":"aa",index=parseInt(colorShade)+1,range=this.contrasts[hasGrey][aa][index];return contrastShade>=range.min&&ontrastShade>=range.max}/**
   * gets the current shade based on the index
   *
   * @param {string} the index
   * @param {number} the shade
   */},{key:"indexToShade",value:function indexToShade(index){return parseInt(index)+1}/**
   * gets the current shade based on the index
   *
   * @param {string} the shade
   * @param {number} the index
   */},{key:"shadeToIndex",value:function shadeToIndex(shade){return parseInt(shade)-1}}],[{key:"tag",get:function get(){return"simple-colors-shared-styles"}}]);return SimpleColorsSharedStyles}(_litElement.LitElement);_exports.SimpleColorsSharedStyles=SimpleColorsSharedStyles;window.customElements.define(SimpleColorsSharedStyles.tag,SimpleColorsSharedStyles);/**
 * Checks to see if there is an instance available, and if not appends one
 */window.SimpleColorsSharedStyles.requestAvailability=function(){if(null==window.SimpleColorsSharedStyles.instance){window.SimpleColorsSharedStyles.instance=document.createElement("simple-colors-shared-styles");window.SimpleColorsSharedStyles.colors=window.SimpleColorsSharedStyles.instance.colors;window.SimpleColorsSharedStyles.contrasts=window.SimpleColorsSharedStyles.instance.contrasts;window.SimpleColorsSharedStyles.stylesheet=document.createElement("style");window.SimpleColorsSharedStyles.stylesheet.innerHTML="".concat(SimpleColorsSharedStyles.styles[0].cssText);document.head.append(window.SimpleColorsSharedStyles.stylesheet)}return window.SimpleColorsSharedStyles.instance}});
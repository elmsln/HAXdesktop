 ...
 */var WebDialog=/*#__PURE__*/function(_HTMLElement){babelHelpers.inherits(WebDialog,_HTMLElement);/**
     * Attaches the shadow root.
     */function WebDialog(){var _this;babelHelpers.classCallCheck(this,WebDialog);_this=babelHelpers.possibleConstructorReturn(this,babelHelpers.getPrototypeOf(WebDialog).call(this));_this.$scrollContainer=document.documentElement;_this.$previousActiveElement=null;var shadow=_this.attachShadow({mode:"open"});shadow.appendChild(template.content.cloneNode(!0));_this.$dialog=shadow.querySelector("#dialog");_this.$backdrop=shadow.querySelector("#backdrop");_this.onBackdropClick=_this.onBackdropClick.bind(babelHelpers.assertThisInitialized(_this));_this.onKeyDown=_this.onKeyDown.bind(babelHelpers.assertThisInitialized(_this));// Set aria attributes
_this.setAttribute("aria-modal","true");_this.$dialog.setAttribute("role","alertdialog");return _this}babelHelpers.createClass(WebDialog,[{key:"connectedCallback",/**
     * Attaches event listeners when connected.
     */value:function connectedCallback(){this.$backdrop.addEventListener("click",this.onBackdropClick)}/**
     * Removes event listeners when disconnected.
     */},{key:"disconnectedCallback",value:function disconnectedCallback(){this.$backdrop.removeEventListener("click",this.onBackdropClick);// If the dialog is open when it is removed from the DOM
// we need to cleanup the event listeners and side effects.
if(this.open){this.didClose()}}/**
     * Shows the dialog.
     */},{key:"show",value:function show(){this.open=!0}/**
     * Closes the dialog with a result.
     * @param result
     */},{key:"close",value:function close(result){this.result=result;this.open=!1}/**
     * Closes the dialog when the backdrop is clicked.
     */},{key:"onBackdropClick",value:function onBackdropClick(){if(this.assertClosing()){this.close()}}/**
     * Closes the dialog when escape is pressed.
     */},{key:"onKeyDown",value:function onKeyDown(e){switch(e.code){case"Escape":if(this.assertClosing()){this.close();// If there are more dialogs, we don't want to close those also :-)
e.stopImmediatePropagation()}break;}}/**
     ...
     */},{key:"assertClosing",value:function assertClosing(){return this.dispatchEvent(new CustomEvent("closing",{cancelable:!0}))}/**
     * Setup the dialog after it has opened.
     */},{key:"didOpen",value:function didOpen(){var _this2=this;// Save the current active element so we have a way of restoring the focus when the dialog is closed.
...
requestAnimationFrame(function(){_this2.$dialog.focusFirstElement()});// Make the dialog focusable
...
this.addEventListener("keydown",this.onKeyDown,{capture:!0,passive:!0});// Increment the dialog count with one to keep track of how many dialogs are currently nested.
...
this.dispatchEvent(new CustomEvent("open"))}/**
     ...
     */},{key:"didClose",value:function didClose(){// Remove the listener listening for key events
this.removeEventListener("keydown",this.onKeyDown,{capture:!0});// Decrement the dialog count with one to keep track of how many dialogs are currently nested.
...
if(0>=(0,_util.getDialogCount)(this.$scrollContainer)){this.$scrollContainer.style.overflow=""}// Make the dialog unfocusable.
this.tabIndex=-1;// Restore previous active element.
if(null!=this.$previousActiveElement){this.$previousActiveElement.focus();this.$previousActiveElement=null}// Dispatch an event so the rest of the world knows the dialog closed.
...
this.dispatchEvent(new CustomEvent("close",{detail:this.result}))}/**
     ...
     */},{key:"attributeChangedCallback",value:function attributeChangedCallback(name,newValue,oldValue){switch(name){case"open":this.open?this.didOpen():this.didClose();break;}}},{key:"open",/**
     * Whether the dialog is opened.
     * @attr
     */get:function get(){return this.hasAttribute("open")},set:function set(value){value?this.setAttribute("open",""):this.removeAttribute("open")}/**
     * Whether the dialog is centered on the page.
     * @attr
     */},{key:"center",get:function get(){return this.hasAttribute("center")},set:function set(value){value?this.setAttribute("center",""):this.removeAttribute("center")}}],[{key:"observedAttributes",get:function get(){return["open","center"]}}]);return WebDialog}(/*#__PURE__*/babelHelpers.wrapNativeSuper(HTMLElement));_exports.WebDialog=WebDialog;customElements.define("web-dialog",WebDialog)});